DROP DATABASE IF EXISTS meta_Lucky_Shrub_my2; 

CREATE DATABASE meta_Lucky_Shrub_my2; 

USE meta_Lucky_Shrub_my2; 
...
-- TASK 1 - Rewrite the SELECT statement so that it is optimized (We only need the OrderID, ProductID, Quantity and Date columns).

SELECT OrderID, ProductID, Quantity, `Date` FROM Orders; -- Now the query is optimized to only show the selected columns
...
EXPLAIN SELECT * FROM Orders WHERE ClientID ='Cl1'; -- You can see that there is no key or possible keys

CREATE INDEX IdxClientID ON orders(ClientID);
EXPLAIN SELECT * FROM Orders WHERE ClientID ='Cl1'; -- Now you will have an index in the column ClientID and the filteres column in the explain table is 100 (greater is ideal for fast queries)
SELECT * FROM Orders WHERE ClientID ='Cl1'; -- After you set an index in the ClientID column and then use this column as the filter the query runs much more fast.
