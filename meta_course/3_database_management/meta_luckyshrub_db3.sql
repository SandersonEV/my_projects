...
FROM employees
WHERE EmployeeID = ANY (SELECT EmployeeID FROM employee_orders WHERE Status = "Completed" ); /* This way is not very good becouse you can only check which EmployeeID is in the list of the subquery in the where clause */
...
ORDER BY COUNT(e.EmployeeName) DESC; /* Use i INNER JOIN look to be the best way because you can also extract other data from the employee_orders table */
...
WHERE HandlingCost > ALL(SELECT ROUND(o.OrderTotal/100 * 20) FROM orders o); 

SELECT e.EmployeeID ID, e.EmployeeName 'Nome do Funcionário'
FROM employees e
...
SELECT EmployeeID ID, EmployeeName 'Nome do Funcionário'
FROM employees
WHERE EmployeeID IN (
SELECT EmployeeID
FROM employee_orders eo
INNER JOIN orders o ON eo.OrderID = o.OrderID
WHERE eo.HandlingCost >= (o.OrderTotal)*0.2
GROUP BY eo.EmployeeID
);

select sum(HandlingCost) from employee_orders group by EmployeeID; 
select SUM(OrderTotal) FROM orders group by OrderID;
